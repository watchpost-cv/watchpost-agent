...
Project status: WP-A01–WP-A05 are complete. Watchpost Agent is a separate
...
`ARCHITECTURE.md` and `PLAN.md` before implementation. Do not reintroduce copied
terminal tokens as the primary pairing UX or model collectors as posts.

WP-A02 provides `cmd/watchpost-agent`, a durable private installation UUID,
...
WP-A05 adds agent-originated pairing through either the website or CLI. Requests
expire after ten minutes and show a matching phrase in both programs. Watchpost
...
WP-A18 adds the composed agent hardening gate, corrupt-state fail-closed proof,
keyboard focus, reduced-motion and forced-colour behavior, and an explicit
limitations file. Linux remains the only supported monitoring platform.
