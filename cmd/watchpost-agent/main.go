...
func localCommand(action string, arguments []string) error {
	...
	case "info":
		current := store.Snapshot()
		result := map[string]any{"installation_id": current.InstallationID, "configured": len(current.LocalAuth.Accounts.Accounts) != 0, "paired": current.Connection.Credential != "", "watchpost_url": current.Connection.WatchpostURL, "post_id": current.Connection.PostID, "queued_batches": len(current.Delivery.Queue), "delivery_failures": current.Delivery.ConsecutiveFailures, "last_delivery_error": current.Delivery.LastError, "dropped_collections": current.Delivery.DroppedCollections}
		if *jsonOutput {
