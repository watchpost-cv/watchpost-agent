...
For unattended setup and management, use `setup` with email/password files, `info --json`, `configure`, `pair`, `pair-status`, `rotate` and `unpair`. Stop the service before destructive recovery. `sudo watchpost-agent reset --auth --confirm "WATCHPOST-AGENT AUTH"` discards even an incompatible authentication payload while preserving valid pairing, collector and installation state. `sudo watchpost-agent reset --all --confirm "WATCHPOST-AGENT ALL"` can replace a damaged state file and returns local state to first run, but does not centrally revoke a lost installation. Both modes retain timestamped backups and preserve service ownership of the active state file. Rebuild or update the installed binary before using commands added by a newer source checkout; invoking `watchpost-agent` uses the installed executable, not source code in the current directory.
